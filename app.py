...
# --- 1. AYARLAR ---
st.set_page_config(page_title="Cotton Trader Pro v27", page_icon="🧶", layout="wide")

# --- 2. TEK VE ANA VERİ MOTORU (TABLO + FİYAT) ---
@st.cache_data(ttl=60)
def get_futures_table_hybrid():
    # ---------------------------------------------------------
    # PLAN A: TradingCharts Sitesini Kazı (Spot Fiyat İçin En İyisi)
    # ---------------------------------------------------------
    ...
    if rows:
        return pd.DataFrame(rows), "Borsa (Yedek)"
...
def ask_gemini_with_chart(api_key, spot_val, news_df, table_df, poly_cent, cot_summary, scenario):
    try:
        ...
        prompt = f"""
        Sen Profesyonel Pamuk Tüccarısın.
        
        BUGÜN: {today_str} | EN YAKIN FİYAT: {spot_val:.2f}c
        RAKİP: Polyester {poly_cent:.2f}c | COT: {cot_summary}
        TABLO: {table_txt}
        HABERLER: {news_df['Orjinal'].to_string() if not news_df.empty else "Yok"}
        SENARYO: {scenario}
        
        GÖREV:
        1. Tablo verilerine dayanarak fiyat yönünü belirle.
        2. Gelecek 1 Yıl için 3 tane fiyat tahmini noktası belirle (JSON formatında).
...
if 'cot_data' not in st.session_state:
    st.session_state['cot_data'] = {"cl": 15000, "cs": 65000, "fl": 40000, "fs": 20000}
...
# EĞER TABLO BOŞSA (ÇOK NADİR), GRAFİĞİN SON FİYATINI KULLAN
if front_price == 0:
    front_price = df_hist['Pamuk'].iloc[-1]
    front_change = front_price - df_hist['Pamuk'].iloc[-2]
    front_name = "Yedek Veri"
...
if menu == "📊 Ana Ekran":
    ...
    with col_l:
        ...
        if not table_data.empty:
            st.dataframe(
                table_data,
                column_config={
                    "Vade": st.column_config.TextColumn("Vade", width="small"),
                    "Son": st.column_config.NumberColumn("Son", format="%.2f"),
    ...
    with col_r:
        ...
        if not news_df.empty:
            st.dataframe(
                news_df[['Duygu', 'Başlık', 'Link']], 
                column_config={
                    "Link": st.column_config.LinkColumn("Kaynak", display_text="🔗"),
                    "Duygu": st.column_config.TextColumn("Yön", width="small"),
                    "Başlık": st.column_config.TextColumn("Haber", width="medium")
                },
                hide_index=True, use_container_width=True
            )
...
# --- EKRAN 2: POZİSYONLAR ---
    ...
    with c2:
        fig_pie = go.Figure(data=[go.Pie(labels=['Fix', 'Tic. Long', 'Fon Long', 'Fon Short'], 
                                         values=[new_cs, new_cl, new_fl, new_fs], hole=.4)])
    ...
    if st.button("Analiz Et") and api_key:
        with st.spinner("AI Fiyat Grafiği Oluşturuyor..."):
            ...
            if not forecast_df.empty:
                st.divider()
                st.subheader("🤖 AI Gelecek Tahmini")
                fig_ai = go.Figure()
...
# --- EKRAN 4: TEKNİK ---
    ...
    show_bb = col_opt2.checkbox("Bollinger", value=True)
    ...
    if show_fib:
        ...
        diff = max_p - min_p
        levels = [(0.0, "Tepe", "red"), (0.5, "Karar", "blue"), (0.618, "Altın", "green"), (1.0, "Dip", "black")]
        for r, l, c in levels:
            p = max_p - r * diff
            fig_tech.add_hline(y=p, line_dash="dash", line_color=c, annotation_text=f"{l} {p:.2f}")
            
    fig_tech.update_layout(height=500, title="Fiyat Analizi", yaxis_title="Cent/lb")
    st.plotly_chart(fig_tech, use_container_width=True)
    
    if show_rsi:
        fig_rsi = go.Figure()
        fig_rsi.add_trace(go.Scatter(x=df_hist.index, y=df_hist['RSI'], name='RSI', line=dict(color='purple')))
        fig_rsi.add_hline(y=70, line_dash="dash", line_color="red", annotation_text="Pahalı")
        fig_rsi.add_hline(y=30, line_dash="dash", line_color="green", annotation_text="Ucuz")
        fig_rsi.update_layout(height=250, title="RSI", yaxis=dict(range=[0, 100]))
